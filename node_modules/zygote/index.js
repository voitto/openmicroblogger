...
var target = data.in_reply_to;
    ...
    squery.on('row', function(row) {
      ...
      var source = 'http://brianhendrickson.com/'+row['currval'];
  ...
  query.on('end', function(result) {
    ...
    /*
        ...
        var query3 = client.query({
          text: "SELECT * FROM mention WHERE reply_url = 'http://brianhendrickson.com/"+rows[0].id+"'",
          values: []
        });
        
        query3.on('row', function(isrep) {
          is_reply = true;
          original_url = 'http://brianhendrickson.com/'+isrep.in_reply_to;
        });
        query3.on('end', function(result) {
console.log("SELECT * FROM mention WHERE reply_url = 'http://brianhendrickson.com/"+rows[0].id+"'");
          if (rows.length > 0) {
            rows[0].is_reply = is_reply;
            rows[0].original_url = original_url;
            rows[0].has_replies = false;
