...
var View,
  ...
  __extends = function(child, parent) { for (var key in parent) { if (__hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; };
  View = (function() {
    function View( mod, req, res, other ) {
      console.log('VIEW SUPER');
      this.response = res;
      this.model = mod;
      this.model.register( this );
      this.init( mod, req, res, other );
    }
    return View;
})();
...
Model.prototype.find = function(id) {
  ...
  var query = client.query({
    text: 'CREATE TABLE post (title text, body text, updated varchar(255), published varchar(255), author varchar(255), author_url varchar(255), author_email varchar(255), profile_image  varchar(255), in_reply_to varchar(255), id serial)',
    values: []
  });
  ...
  /*
  var query = client.query({
    text: 'ALTER TABLE post add column in_reply_to varchar(255)',
    values: []
  });
  query.on('error', function(row) {
  });
...
console.log("SELECT * FROM mention WHERE reply_url = prefs['url']+'/"+rows[0].id+"'");
          if (rows.length > 0) {
            rows[0].is_reply = is_reply;
            rows[0].original_url = original_url;
            rows[0].has_replies = false;
...
View.prototype.render = function() {
  ...
  data['content'] = ''+mustache.to_html( t, data );
...
get('/lib.js', function(req,res){
  var ctype = { 'Content-Type' : 'application/javascript' };
  res.writeHead( 200, ctype );
  res.end(mustache.to_html( fs.readFileSync( __dirname + '/../../' + 'lib.js', 'utf-8' ), {url:prefs['url'],signed_in:signed_in} ));
...
get('/app.js', function(req,res){
  var ctype = { 'Content-Type' : 'application/javascript' };
  res.writeHead( 200, ctype );
  res.end(mustache.to_html( fs.readFileSync( __dirname + '/../../' + 'app.js', 'utf-8' ), {url:prefs['url'],signed_in:signed_in} ));
});
...
get('/style.css', function(req,res){
  var ctype = { 'Content-Type' : 'text/css' };
  res.writeHead( 200, ctype );
...
post('/person/signin', function(req,res) {
  ...
  req.on('end', function() {
    ...
    client.connect();
    var query = client.query({
      text: 'SELECT password,email FROM person WHERE email ilike $1',
      values: [data.email]
    });
    query.on('row', function(row) {
      rows.push(row);
