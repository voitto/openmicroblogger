    ...
    squery.on('row', function(row) {
      ...
      console.log('posting to '+parsed.hostname+' '+port.toString()+parsed.path);
        ...
        pingback.send(url.parse(target), url.parse(source), function(err, pingback) {
          if (!err) console.log('Pinged ' + pingback.href + ' successfully.');
          else console.log('Pingback try but no' +JSON.stringify(pingback)+ ' t='+target+' s='+source);
        });
        ...
        http.get(options, function(res) {
          ...
          res.on('end', function (chunk) {
            ...
            pingback.scan(data, source, function(err, pingback) {
              if (!err) console.log('Pinged ' + pingback.href + ' successfully.');
              else console.log('Pinged ' + source + ' but it failed sorry.');
            });
          });
          ...
          console.log('ERROR: ' + e.message + JSON.stringify(options));
        });
      */
...
get('/lib.js', function(req,res){
  res.end(mustache.to_html( fs.readFileSync( __dirname + '/../../' + 'lib.js', 'utf-8' ), {url:prefs['url']} ));
    ...
    query.on('end', function(result) {
        ...
        query2.on('end', function(result) {
          ...
          query3.on('row', function(isrep) {
            is_reply = true;
            original_url = prefs['url']+'/'+isrep.in_reply_to;
            console.log(original_url+' <url ROW+'+JSON.stringify(isrep));
          });
          query3.on('end', function(result) {
            if (rows.length > 0) {
              rows[0].is_reply = is_reply;
