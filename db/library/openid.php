...
class SimpleOpenID{
    ...
    function SetIdentity($a){ 	// Set Identity URL
        ...
        if ((in_string('http://',$a) === false)
          && (in_string('https://',$a) === false)){
            $a = 'http://'.$a;
        }
        if (in_string('gmail',$a) || in_string('google',$a)){
           $a = "http://gmail.com";
        }        
    ...
    function GetRedirectURL(){
        ...
        if (isset($this->fields['required']) || isset($this->fields['optional'])) {
            ...
            $params['openid.ns.sreg'] = urlencode("http://openid.net/extensions/sreg/1.1");
        }
        ...
        if (isset($this->fields['pape_policies']) 
          && (count($this->fields['pape_policies']) > 0)) {
