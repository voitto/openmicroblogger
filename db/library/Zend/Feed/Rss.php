...
//        $channel->appendChild($objecttype);
    ...
    protected function _mapFeedEntries(DOMElement $root, $array)
    {
        ...
        Zend_Feed::registerNamespace('activity', 'http://activitystrea.ms/spec/1.0/');
        Zend_Feed::registerNamespace('atom', 'http://www.w3.org/2005/Atom');
        Zend_Feed::registerNamespace('ostatus', 'http://ostatus.org/schema/1.0');
        Zend_Feed::registerNamespace('georss', 'http://www.georss.org/georss');
        Zend_Feed::registerNamespace('poco', 'http://portablecontacts.net/spec/1.0');
        Zend_Feed::registerNamespace('media', 'http://purl.org/syndication/atommedia');

        foreach ($array as $dataentry) {
            $item = $this->_element->createElement('item');

//		        $object = $this->_element->createElement('activity:object');


//		        $objecttype = $this->_element->createElement('activity:object-type','http://activitystrea.ms/schema/1.0/note');
...
if ($arr){
  ...
  $pic->setAttribute('href', $arr[0]);
  $pic->setAttribute('type', $arr[1]);
  //$pic->setAttribute('width', $arr[3]);
  //$pic->setAttribute('height', $arr[4]);
  $item->appendChild($pic);

	if ($arr[5]){
	  $tpic = $this->_element->createElement('atom:link');
	  $tpic->setAttribute('rel', 'preview');
	  $tpic->setAttribute('href', $arr[5]);
	  $tpic->setAttribute('type', $arr[1]);
	  //$tpic->setAttribute('width', $arr[6]);
	  //$tpic->setAttribute('height', $arr[7]);
	  $item->appendChild($tpic);
	}
...
//					 <thr:in-reply-to ref="http://identi.ca/notice/43490390" href="http://identi.ca/notice/43490390"></thr:in-reply-to>

           $conv = $this->_element->createElement('ostatus:conversation', $dataentry->link);
           $item->appendChild($conv);
...
//		        $item->appendChild($object);
    ...
    public function saveXml()
    {
        ...
        // Content namespace
        $root->setAttributeNS('http://www.w3.org/2000/xmlns/', 'xmlns:ostatus', 'http://ostatus.org/schema/1.0');
        $root->setAttributeNS('http://www.w3.org/2000/xmlns/', 'xmlns:georss', 'http://www.georss.org/georss');
        $root->setAttributeNS('http://www.w3.org/2000/xmlns/', 'xmlns:poco', 'http://portablecontacts.net/spec/1.0');
        $root->setAttributeNS('http://www.w3.org/2000/xmlns/', 'xmlns:media', 'http://purl.org/syndication/atommedia');
        $root->setAttributeNS('http://www.w3.org/2000/xmlns/', 'xmlns:content', 'http://purl.org/rss/1.0/modules/content/');
			  $root->setAttributeNS('http://www.w3.org/2000/xmlns/', 'xmlns:activity', 'http://activitystrea.ms/spec/1.0/');
