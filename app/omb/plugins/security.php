	  ...
	  if (!isset($_SESSION['googleAccessKey']) && !isset($_SESSION['googleAccessSecret'])){
	    if ($request->oauth_verifier)
		  ...
		  if (!isset($request->oauth_token)){
			  ...
			  $req_req = get_oauth_request(NULL,$endpoints[0], array('oauth_callback'=>$callback_url,'domain'=>$request->domain) );
			  ...
			  $auth_url = $endpoints[1] . "?oauth_token=$token&oauth_callback=".urlencode($callback_url).'&scope='.$scope.'&domain='.$request->domain;
			  redirect_to($auth_url);
		  } else {
			  $token = get_oauth_token($request->oauth_token,$_SESSION['google_secret']);
			  $acc_req = get_oauth_request($token,$endpoints[2],array('oauth_verifier'=>$_SESSION['google_verifier'],'domain'=>$request->domain));
